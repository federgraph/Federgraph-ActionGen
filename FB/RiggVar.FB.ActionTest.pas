...
type
  ...
  protected
    ...
    function TestEncodedValues: Boolean;
    function TestDecodedValues: Boolean;
  public
    WantNew: Boolean;
    SL: TStringList;
    constructor Create;
    ...
    begin
      ...
      case af of
        ...
        begin
          ...
          begin
            ad := 'fag' + Copy(an, 3);
            ML.Add(Format('%s%s: result := %s;', [Indent4, an, ad]));
          end;
        end;
        afDecoded:
        begin
          ...
          begin
            ad := 'fag' + Copy(an, 3);
            ML.Add(Format('%s%s: result := %s;', [Indent4, ad, an]));
          end;
        end;
      end;
    end;
...
function TActionTest.TestEncodedValues: Boolean;
var
  fa: Integer;
  fag: Integer;
  t: string;
begin
  result := True; // assume OK
  SL.Add('Test for missing Encoded Values:');
  for fa := 0 to faMax - 1 do
  begin
    if fa = faNoop then
      Continue;
    fag := GetFederActionEncodedValue(fa);
    if fag = -1 then
    begin
      result := False;
      t := GetFederActionName(fa);
...
function TActionTest.TestDecodedValues: Boolean;
var
  fa, fa2: Integer;
  fag: Integer;
  t: string;
begin
  result := True; // assume OK
  SL.Add('Test for missing Decoded Values:');
  for fa := 0 to faMax - 1 do
  begin
    if fa = faNoop then
      Continue;
    fag := GetFederActionEncodedValue(fa);
    fa2 := GetFederActionDecodedValue(fag);
    if fa2 = -1 then
    begin
      result := False;
      t := GetFederActionName(fa);
